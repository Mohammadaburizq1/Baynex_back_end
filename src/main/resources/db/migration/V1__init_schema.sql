...
CREATE TABLE app_users (
    ...
    phone VARCHAR(40),
    -- Explicitly named: matches the name Postgres would auto-generate for an unnamed inline
    -- CHECK here anyway (<table>_<column>_check), but H2 auto-generates a different name, and
    -- V9__production_auth_hardening.sql's DROP CONSTRAINT needs both DBs to agree on it.
    role VARCHAR(30) NOT NULL CONSTRAINT app_users_role_check CHECK (role IN ('SUPER_ADMIN','MERCHANT','CUSTOMER')),
